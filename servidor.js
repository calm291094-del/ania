...
/* ==================== CACHÉ GITHUB ==================== */
const GHCache = {
  store: new Map(), TTL: 30000,
  get(k){ const e = this.store.get(k); if (!e) return null; if (Date.now() - e.t > this.TTL){ this.store.delete(k); return null; } return e.value; },
  set(k, v){ this.store.set(k, { value:v, t:Date.now() }); },
...
async function ghRead(p){
  ...
  if (r.status === 404){ GHCache.set('read:' + p, null); return null; }
...
/* ==================== RATE LIMITERS ==================== */
const limiterLogin = rateLimit({ windowMs: 15*60*1000, max: 5, standardHeaders: true, legacyHeaders: false, message:{ error:'demasiados intentos, espera 15 minutos' }, skipSuccessfulRequests: true });
const limiterRegister = rateLimit({ windowMs: 60*60*1000, max: 3, standardHeaders: true, legacyHeaders: false, message:{ error:'demasiados registros, espera 1 hora' } });
const limiterPassword = rateLimit({ windowMs: 60*60*1000, max: 5, standardHeaders: true, legacyHeaders: false, message:{ error:'demasiados intentos de contraseña' } });
const limiterGeneral = rateLimit({ windowMs: 15*60*1000, max: 300, standardHeaders: true, legacyHeaders: false, message:{ error:'demasiadas peticiones' } });
...
app.post('/ania/register', limiterRegister, async (req,res)=>{
  try{
    ...
    if (!usuario || !password || !nombre || !email) return res.status(400).json({ error:'faltan campos' });
    if (!/^[a-zA-Z0-9_.-]{3,20}$/.test(usuario)) return res.status(400).json({ error:'usuario inválido' });
